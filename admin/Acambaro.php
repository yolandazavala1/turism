<?php
  ...
  if (isset($_SESSION['user_id'])) {
	...
	$records = $conn->prepare('SELECT * FROM Acambaro where id=1');
    $records->bindParam(':id', $_SESSION['user_id']);
	$records->execute();
	$results = $records->fetch(PDO::FETCH_ASSOC);
	$user = null;
	...
	$records = $conn->prepare('SELECT * FROM Acambaro where id=2');
    $records->bindParam(':id', $_SESSION['user_id']);
	$records->execute();
	$results = $records->fetch(PDO::FETCH_ASSOC);
	$user2 = null;
	...
	$records = $conn->prepare('SELECT * FROM Acambaro where id=3');
    $records->bindParam(':id', $_SESSION['user_id']);
	$records->execute();
	$results = $records->fetch(PDO::FETCH_ASSOC);
	$user3 = null;
    ...
    $records = $conn->prepare('SELECT * FROM Acambaro where id=4');
    $records->bindParam(':id', $_SESSION['user_id']);
	$records->execute();
	$results = $records->fetch(PDO::FETCH_ASSOC);
	$user4 = null;
	...
	<nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
       ...
       <div class="col-md-6">
                <div class="card text-white">
                    <img src="../img/ac1.jpg" class="card-img-top" alt="...">
                    <div class="card-img-overlay d-flex align-items-end">
                        <div class="card-body"  style="background-image: linear-gradient(rgba(252, 252, 252, 0.1),rgba(32, 32, 32, 0.8));">
                            ...
                            <?php if($users['admin']==1): ?>
                                <a href="editarAcambaro.php">Editar</a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
			...
			<div class="col-md-6">
                <div class="card text-white">
                    <img src="../img/ac2.jpg" class="card-img-top" alt="...">
                    <div class="card-img-overlay d-flex align-items-end">
                        <div class="card-body" style="background-image: linear-gradient(rgba(252, 252, 252, 0.1),rgba(32, 32, 32, 0.8));">
